...
- **ASCII-only scripts**: All `.ps1` and automation scripts must be ASCII-only;
  no emoji or Unicode in script files
...
- **Toolchain**: Quarto RevealJS for all slide decks (HTML output, version-controlled
  ...
  callouts, speaker notes, section breaks; slides are self-sufficient and assets are
...
every slide must advance that argument, and every minute on stage must earn the audience's
attention through clarity, evidence, and respect for their expertise.
